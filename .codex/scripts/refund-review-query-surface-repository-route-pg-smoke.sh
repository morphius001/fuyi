...
smoke_script="$root/.codex/scripts/refund-review-query-surface-repository-route-pg-smoke.ts"
work_dir="${TMPDIR:-/tmp}/fuyi-refund-review-query-surface-route-pg-smoke"
...
case "$env_hint" in
  *production*|*prod*|*preprod*|*staging*)
    echo "Refusing repository route PG smoke while environment hints at production, preprod, or staging." >&2
    exit 1
    ;;
esac

if grep -Eq 'Migration20260512000300|Migration20260514000100|china_refund_state_mutation_(approval|audit|runtime_attempt|terminal_conflict)' "$root/packages/api/medusa-config.ts"; then
  echo "Refusing route PG smoke: review query surface migrations appear registered in medusa-config.ts." >&2
  exit 1
fi
...
command -v node >/dev/null || {
  ...
  if [ -s "$NVM_DIR/nvm.sh" ]; then
    ...
    nvm use >/tmp/fuyi-nvm-use-refund-review-route-pg-smoke.log
  fi
}
...
echo "CREATE disposable refund review query surface route PG smoke database: $db_name"
...
echo "SEED repository route PG smoke fixtures"
...
echo "RUN repository route PG smoke"
cd "$root"
bun run "$smoke_script" "$database_url"
...
echo "PASS refund review query surface repository route PG smoke completed for database: $db_name"
