...
新增支付宝 fake notify normalizer 纯函数合同，把 verifier result + fake form 映射为标准 payment notification envelope。

## 范围

- 新增 `normalizeAlipayNotificationContract()`。
- 新增 `parseAlipayAmountToMinorUnitsContract()`。
- 只处理 fake verified `TRADE_SUCCESS` / `TRADE_CLOSED` notification。
...
- `.codex/scripts/payment-notification-idempotency-harness.sh`
- `docs/alipay-notification-normalizer-contract.md`
- ledger / queue 更新
