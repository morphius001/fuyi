...
新增支付宝 Provider disabled adapter skeleton，作为后续 sandbox fake notify、验签测试和 runtime gate 前的代码边界。

## 范围

- 新增未注册的支付宝 disabled adapter 纯函数 skeleton。
- 暴露配置 key 名和 secret reference key 名，不读取环境变量或 secret。
- `createPayment` / `queryPayment` / `closePayment` / `verifyNotification` / `normalizeNotification` 均返回 blocked decision。
...
- 不接 checkout，不返回真实 payment URL、QR code 或支付宝调起 payload。
...
- `.codex/scripts/payment-notification-idempotency-harness.sh`
- `docs/alipay-provider-disabled-adapter-skeleton.md`
- ledger / queue 更新
