...
新增退款 manual review audit 纯函数和 focused tests，只输出不可执行审计 decision。

## 范围

- 新增 `refund-manual-review-audit.ts`。
...
TEST_TYPE=unit NODE_OPTIONS=--experimental-vm-modules ./node_modules/.bin/jest --silent --runInBand --forceExit --runTestsByPath src/modules/china-payment-notification/__tests__/refund-manual-review-audit.unit.spec.ts
bunx tsc --noEmit -p tsconfig.json
cd /home/codex/code/fuyi-pr-bx-workflow-handoff-cn
.codex/scripts/payment-notification-idempotency-harness.sh
grep -RIn -e refundStateMutation -e providerRefundRequest -e createRefund -e wechat_refund -e alipay_refund -e workflow_execution packages/api/src/modules/china-payment-notification/refund-manual-review-audit.ts packages/api/src/modules/china-payment-notification/__tests__/refund-manual-review-audit.unit.spec.ts packages/api/src/api/china || true
git diff --check
```
