...
新增退款通知 fake-only verifier 纯函数和 focused tests，覆盖 `refund.succeeded` / `refund.failed` 的 fake fixture 验签合同。

## 范围

- 新增 `refund-notification-verifier.ts`。
...
TEST_TYPE=unit NODE_OPTIONS=--experimental-vm-modules ./node_modules/.bin/jest --silent --runInBand --forceExit --runTestsByPath src/modules/china-payment-notification/__tests__/refund-notification-verifier.unit.spec.ts
bunx tsc --noEmit -p tsconfig.json
cd /home/codex/code/fuyi-pr-bx-workflow-handoff-cn
.codex/scripts/payment-notification-idempotency-harness.sh
grep -RIn -e refundStateMutation -e providerRefundRequest -e createRefund -e wechat_refund -e alipay_refund -e workflow_execution packages/api/src/modules/china-payment-notification/refund-notification-verifier.ts packages/api/src/modules/china-payment-notification/__tests__/refund-notification-verifier.unit.spec.ts packages/api/src/api/china || true
git diff --check
```
