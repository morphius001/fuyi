...
验证 PR #428 `refund-state-mutation-approval-persistence-plan` 合并后的真实状态：

- approval persistence plan 仍为 docs-only。
...
真实生产退款成功状态写入仍 No-Go。下一步进入 `refund-state-mutation-approval-persistence-contract`，只能新增不可执行 approval persistence intent 纯函数和 focused tests。
