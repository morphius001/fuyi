...
验证已合并的 `refund-state-mutation-operator-approval-contract` PR，确认 operator approval 合同仍保持不可执行、不写退款成功状态。

## 范围

- 记录 PR #409 合并 commit 和文件范围。
- 运行 focused test、API typecheck、payment notification harness、runtime grep 和 `git diff --check`。
- 新增 `docs/refund-state-mutation-operator-approval-validation.md`。
