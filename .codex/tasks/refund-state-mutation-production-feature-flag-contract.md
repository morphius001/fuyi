...
新增 production feature flag / kill switch 的 disabled / non-executable 纯函数合同。

## 范围

- 新增 `packages/api/src/modules/china-payment-notification/refund-state-mutation-production-feature-flag.ts`。
- 新增 focused unit tests。
- 从 `packages/api/src/modules/china-payment-notification/index.ts` 导出。
- 更新 payment notification harness。
- 新增 `docs/refund-state-mutation-production-feature-flag-contract.md`。
