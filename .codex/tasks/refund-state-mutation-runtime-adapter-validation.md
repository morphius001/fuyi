...
验证已合并的 `refund-state-mutation-runtime-adapter-contract` PR，确认 runtime adapter 合同仍 disabled / non-executable。

## 范围

- 记录 PR #413 合并 commit 和文件范围。
- 运行 focused test、API typecheck、payment notification harness、runtime grep 和 `git diff --check`。
- 新增 `docs/refund-state-mutation-runtime-adapter-validation.md`。
