...
新增 runtime attempt persistence repository 的 disabled / non-executable 合同层，先定义 repository record、event、查询接口和 repository intent 纯函数，不连接 production DB、不写 runtime attempt persistence record、不执行 workflow。
