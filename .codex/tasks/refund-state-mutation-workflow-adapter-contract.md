...
新增 audit write intent 到 refund workflow command adapter 的 disabled / non-executable 纯函数合同。

## 范围

- 新增 `packages/api/src/modules/china-payment-notification/refund-state-mutation-workflow-adapter.ts`。
- 新增 focused unit tests。
- 从 `packages/api/src/modules/china-payment-notification/index.ts` 导出。
- 更新 payment notification harness。
- 新增 `docs/refund-state-mutation-workflow-adapter-contract.md`。
