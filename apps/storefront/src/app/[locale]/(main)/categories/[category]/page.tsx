...
export async function generateMetadata({
  ...
  const title = `${cat.name} 类目`;
  const description = `${cat.name} 类目 - ${process.env.NEXT_PUBLIC_SITE_NAME || 'Fuyi 买家端'}`;
  ...
  return {
    ...
    openGraph: {
      title: `${title} | ${process.env.NEXT_PUBLIC_SITE_NAME || 'Fuyi 买家端'}`,
      description,
      url: canonical,
      siteName: process.env.NEXT_PUBLIC_SITE_NAME || 'Fuyi 买家端',
      type: 'website'
    }
  };
}
...
async function Category({
  ...
  const currency_code = (await getRegion(locale))?.currency_code || 'cny';
