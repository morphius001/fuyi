...
const returnStatuses = ["pending", "processing", "sent"];
const steps = ["已提交", "处理中", "已寄出"];
...
export const SingleOrderReturn = ({
  ...
  const currentStep = returnStatuses.indexOf(item.status);

  return (
    <>
      <Card
        className="bg-secondary p-4 flex justify-between mt-8"
      ...
      >
        ...
        >
          订单：#{item.order.display_id}
        </Heading>
        <div className="flex flex-col gap-2 items-center">
          <p
            className="label-sm text-secondary"
            data-testid={
              testIdPrefix ? `${testIdPrefix}-requested-date` : undefined
            }
          >
            申请时间： {format(item.line_items[0].created_at, "MMM dd, yyyy")}
          </p>
        </div>
      </Card>
      <Card
      ...
      >
        ...
        >
          ...
          <p
            className="label-sm text-secondary flex gap-2"
            data-testid={
              testIdPrefix ? `${testIdPrefix}-items-count` : undefined
            }
          >
            {item.line_items.length}{" "}
            {item.line_items.length > 1 ? "件商品" : "件商品"}
            <CollapseIcon
              className={cn(
          ...
          <div className="p-4 flex justify-between">
            <div className="flex items-center gap-2">
              ...
              <p
                ...
                data-testid={
                  testIdPrefix ? `${testIdPrefix}-seller-name` : undefined
                }
              >
            ...
            <div className="flex flex-col gap-4 w-full">
              {filteredItems.map((filteredItem: any) => (
                <div
                  ...
                  data-testid={
                    testIdPrefix
                      ? `${testIdPrefix}-item-${filteredItem.id}`
                      : undefined
                  }
                >
                  <div className="flex items-center gap-4 w-1/2">
                    ...
                    <div>
                      <p
                        ...
                        data-testid={
                          testIdPrefix
                            ? `${testIdPrefix}-item-${filteredItem.id}-title`
                            : undefined
                        }
                      >
                  ...
                  <div className="flex justify-between w-1/2">
                    <p
                      ...
                      data-testid={
                        testIdPrefix
                          ? `${testIdPrefix}-item-${filteredItem.id}-reason`
                          : undefined
                      }
                    >
                    ...
                    <p
                      ...
                      data-testid={
                        testIdPrefix
                          ? `${testIdPrefix}-item-${filteredItem.id}-price`
                          : undefined
                      }
                    >
                      {convertToLocale({
