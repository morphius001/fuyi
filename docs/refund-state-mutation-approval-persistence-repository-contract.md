...
本轮新增 approval persistence repository 的 disabled / non-executable 合同层。当前只定义 record、event、查询接口和 repository intent 纯函数，不连接 production DB、不写 approval record、不执行 workflow、不写 production refund success state。
...
1. `RefundStateMutationApprovalPersistenceRecord`
2. `RefundStateMutationApprovalPersistenceEventRecord`
3. `RefundStateMutationApprovalPersistenceRepositoryContract`
4. `mapApprovalPersistenceToRepositoryIntent()`
...
- approval persistence 输入若不安全或缺少 persistence intent，repository contract 直接 blocked / rejected。
- 当前 contract 只输出 disabled repository intent 和 audit event，不代表可写 approval 主表或 event 表。
...
- 不读取或写入 `china_refund_state_mutation_approval` 表
...
建议进入 `refund-state-mutation-approval-persistence-repository-validation`，只做合并后验证和 No-Go 收口。
