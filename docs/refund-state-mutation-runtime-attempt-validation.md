...
验证 PR #442 `refund-state-mutation-runtime-attempt-plan` 合并后的真实状态：

- runtime attempt plan 仍为 docs-only。
...
`refund-state-mutation-runtime-attempt-plan` 只规划：

- workflow attempt persistence schema。
- attempt status model。
- idempotency / replay rules。
- schema planning notes。
- runtime boundaries。

真实生产退款成功状态写入仍 No-Go。下一步进入 `refund-state-mutation-runtime-attempt-contract`。
