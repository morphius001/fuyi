# Refund State Mutation Terminal Conflict Persistence Repository Contract

更新时间：2026-05-13 Asia/Shanghai

## 结论

本轮新增 terminal conflict persistence repository 的 disabled / non-executable 合同层。当前只定义 snapshot、event、查询接口和 repository intent 纯函数，不连接 production DB、不写 terminal conflict snapshot、不执行 workflow、不写 production refund success state。

## Files Changed

- `packages/api/src/modules/china-payment-notification/refund-state-mutation-terminal-conflict-persistence-repository.ts`
- `packages/api/src/modules/china-payment-notification/__tests__/refund-state-mutation-terminal-conflict-persistence-repository.unit.spec.ts`
- `packages/api/src/modules/china-payment-notification/index.ts`
- `docs/refund-state-mutation-terminal-conflict-persistence-repository-contract.md`
- `.codex/tasks/refund-state-mutation-terminal-conflict-persistence-repository-contract.md`
- `.codex/queue.md`
...
1. `RefundStateMutationTerminalConflictPersistenceSnapshotRecord`
2. `RefundStateMutationTerminalConflictPersistenceEventRecord`
3. `RefundStateMutationTerminalConflictPersistenceRepositoryContract`
4. `mapTerminalConflictToRepositoryIntent()`
...
- terminal conflict 输入若不安全或缺少 intent，repository contract 直接 blocked / rejected。
- 当前 contract 只输出 disabled repository intent 和 audit event，不代表可写 terminal conflict snapshot 主表或 event 表。
...
- 不读取或写入 terminal conflict persistence 表
