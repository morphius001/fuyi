...
const resolveVisibleSellerProductIds = async (req) => {
    ...
    const rows = await pg("product_product_seller_seller")
        ...
        .andWhere((builder) => {
            builder.whereNull("seller.closed_from").orWhere("seller.closed_from", ">", now);
        })
        .andWhere((builder) => {
            builder.whereNull("seller.closed_to").orWhere("seller.closed_to", "<", now);
        })
...
const applySellerVisibilityAsProductFilter = async (req, _res, next) => {
    try {
        ...
        req.filterableFields.id = intersectIds(req.filterableFields.id, visibleProductIds);
        delete req.filterableFields.seller;
