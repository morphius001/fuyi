...
1. `refund-state-mutation-isolated-preprod-query-surface-fixture-registry-reference-slot-contract-validation` 已完成，reference slot contract plan 的 docs-only 边界已经收口。
2. 本轮主要规划 timeline event 的字段结构、排序键、timestamp redaction 和 event reference handle 合同。
3. 下一步默认进入 `refund-state-mutation-isolated-preprod-query-surface-fixture-registry-timeline-event-contract-validation`，确认 timeline event contract plan 仍然只是 docs-only 规划。
