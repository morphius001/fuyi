...
1. `refund-state-mutation-isolated-preprod-query-surface-fixture-registry-cross-block-consistency-validation` 已完成，cross block consistency 的 docs-only 收口已经进入主线。
2. 本轮主要规划 payload version、mixed bundle、partial upgrade 和 unsupported shape 的 fail-closed 策略。
3. 下一步默认进入 `refund-state-mutation-isolated-preprod-query-surface-fixture-registry-payload-compatibility-validation`，确认本轮 compatibility plan 仍然只是 docs-only 规划。
